...
export const Extension = (props) => {
    ...
    React.useEffect(() => {
        const loadConfigs = async () => {
            ...
            for (const extension of extensions) {
                if (newExtensions.find(parcel => parcel.extension.appName === extension.appName))
                    return;
                try {
    ...
    const handleParcelError = React.useCallback((extension) => (err) => {
        if (logger)
            logger.error(`Error in ${extension.appName}_${name}: ${err}`);
        onError?.(extension, `Failed to mount. ${err.message}`);
    }, [logger, name, onError]);
    const isLoading = extensions.length > parcelConfigs.length && !isEmpty;
    return (React.createElement(Stack, { customStyle: customStyle, id: name },
        isLoading && loadingIndicator,
        isEmpty && emptyIndicator,
        parcelConfigs.map(parcel => (React.createElement(RootParcel, { key: parcel.extension.appName, config: { ...parcel.config, name: `${parcel.extension.appName}_${name}` }, ...getContext(), extensionData: extensionData, handleError: handleParcelError(parcel.extension) })))));
};
